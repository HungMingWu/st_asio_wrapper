...
public:
	...
	#else
	void post(const std::function<void()>& handler) {io_context_.post((aci, boost::lambda::bind(boost::lambda::unlambda(handler))));}
	void dispatch(const std::function<void()>& handler) {io_context_.dispatch((aci, boost::lambda::bind(boost::lambda::unlambda(handler))));}
	void post_strand(boost::asio::io_context::strand& strand, const std::function<void()>& handler)
